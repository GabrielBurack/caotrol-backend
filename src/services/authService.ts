...
class AuthService {
  async login(login?: string, senha?: string): Promise<string> {
    ...
    if (user.id_veterinario) {
      ...
      const veterinario = await veterinarioRepository.findById(
        user.id_veterinario
      );
      if (veterinario?.nome) {
  ...
  async verifyEmail(token: string) {
    ...
    const user = await userRepository.findByVerificationToken(token); // (Precisaremos criar este método no repositório)

    if (!user) {
      throw new UnauthorizedError("Token de verificação inválido ou expirado.");
    }

    // Atualiza o usuário para o status verificado e limpa o token
    await userRepository.update(user.id_usuario, {
  ...
  async resetPassword(token: string, novaSenha: string) {
    if (!token || !novaSenha)
      throw new BadRequestError("O token e a nova senha são obrigatórios.");
